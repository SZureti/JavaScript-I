...
//also
console.log('the last car in inventory:', inventory[inventory.length - 1]);
...
let carModels = inventory.car_model.map;
...
let carYears = inventory.car_year.map;
...
let BMWAndAudi = function BMWAndAudi(){
    let x = car_make;
    if(x == BMW || x == Audi){
        return car_make;
    }
